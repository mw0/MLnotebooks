...
def learningCurve(model, train, test, epochs, outFile=None,
                  k=5, showProgress=True, numThreads=0):
    # if not userIndex:
    #     userIndex = range(train.shape[0])
    prevEpoch = 0
    ...
    for epoch in epochs:
        ...
        trainTcsr = train.T.tocsr()
        testTcsr = test.T.tocsr()

        pAtK.append(precision_at_k(model, trainTcsr, testTcsr,
                                   K=k, show_progress=showProgress,
                                   num_threads=numThreads))
        MAPatK.append(mean_average_precision_at_k(model, trainTcsr,
                                                  testTcsr, K=k,
                                                  show_progress=showProgress,
                                                  num_threads=numThreads))
        NDCGatK.append(ndcg_at_k(model, trainTcsr, testTcsr,
                                 K=k, show_progress=showProgress,
                                 num_threads=numThreads))
        AUCatK.append(AUC_at_k(model, trainTcsr, testTcsr,
                               K=k, show_progress=showProgress,
                               num_threads=numThreads))
...
ex.observers.append(MongoObserver(url='localhost:27017', db_name='Implicit'))
...
def my_metrics(_run, logs):
      ...
      _run.log_scalar("lr", float(logs.get('lr')))
...
def run(modelName, datasetName, factorCt, k, λ, α,
    ...
    _, pAtK, MAPatK, NDCGatK, AUCatK = \
        learningCurve(model, train, test, epochs, k=k,
                      outFile=outFile, showProgress=showProgress,
                      numThreads=numThreads,
    ...
    # print(f"Computing AUC@{k} ...", flush=True)
    # t0 = time()
